...
The personal workspace uses constrained section widths, compact profile headings, uniform 46px profile controls, readable credential/session lists and application tables. Success/error notices and confirmation dialogs share the same visual treatment. Navigation has keyboard focus states and reduced-motion support. The native mobile account menu remains accessible for logout; do not hide its toolbar item while customizing the header. Account resource URLs are versioned in `amigo/account/theme.properties`; rebuild the image after theme edits.

The main workspace and right account header share a subtle dotted material background, separated by a full-width light-gray line below the header. The brand/menu area and sidebar share one light-gray rail. Sidebar transitions follow native Keycloak expanded/collapsed classes, use desktop width animation and mobile slide animation, and respect reduced-motion preferences. Collapsed navigation is inert so hidden links cannot receive keyboard focus.
